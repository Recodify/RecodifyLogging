...
#>
	...
	Process {
				...
				if ($gitRepoPath -eq "")
				{
					...
					Write-Host "Setting Git repository path to the calling scripts path (using Resolve-Path .): $gitRepoPath"
				}
				...
				$assemblyVersion = [string]::Format("{0}.{1}.{2}", $major, $minor, 0) #AssemblyVersion
				$assemblyFileVersion = [string]::Format("{0}.{1}.{2}", $major, $minor, $buildCounter) #AssemblyFileVersion
				$assemblyInformationalVersion = [string]::Format("{0}.{1}.{2}-{3}{4}", $major, $minor, $buildCounter, $branchName.ToLower(), $gitCommitIdentifier.ToLower()) 
				...
				{
					Update-Info $assemblyInfoFiles $newAssemblyVersion $newAssemblyFileVersion $newAssemblyInformationalVersion
				}
				
				$solutionInfoFiles = Get-ChildItem $gitRepoPath -recurse -include SolutionInfo.cs				
				if ($solutionInfoFiles -ne $null) 
				{				
					Update-Info  $solutionInfoFiles $newAssemblyVersion $newAssemblyFileVersion $newAssemblyInformationalVersion
				}
				...
				if ($sendTeamCityServiceMessage -eq $true)
				{
					...
					Write-Host "##teamcity[buildNumber '$assemblyInformationalVersion']" 
				}
...
function Update-Info {
	Param(
		...
		[Parameter(Mandatory = $True )]
		[string] $newAssemblyInformationalVersion
	)
	
	ForEach ($assemblyInfoFile in $infoFiles)
	{
		Try	{
...
function Set-Revision {
	...
	Write-Host "Setting revision number to: $revision"
...
function Set-GitPath {
	...
	if ($gitPath -eq "")
	{
		...
		$gitPath = "git.exe"
		Write-Host "Assuming (default) Git.exe is included in the Windows path environment variable: $gitPath"
		return $gitPath
	}
	else
	{
		...
		Write-Host "Git path set as: $gitPath"
	}					
