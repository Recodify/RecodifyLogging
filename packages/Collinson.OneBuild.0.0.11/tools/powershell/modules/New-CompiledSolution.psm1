...
#>
	...
	Process {
				...
				{					
					...
					$result = Invoke-MsBuildCompilationForSolution -solutionFile $solutionFile -configMode $configMode -assemblyInformationVersion $assemblyInformationVersion
					if ($result -ne $null) 
					{
...
function Invoke-MsBuildCompilationForSolution {
	Param(			
		...
		[Parameter(Mandatory = $True )]
			[string]$configMode,
		[Parameter(Mandatory = $True)]
			[string] $assemblyInformationVersion			
	)
	Write-Warning "Building '$($solutionFile)' in '$($configMode)' mode"	
	Write-Host $assemblyInformationVersion
	$output = (& $msbuildPath $solutionFile /t:ReBuild /t:Clean /p:Configuration=$configMode /p:OctoPackPackageVersion="$assemblyInformationVersion" /p:RunOctoPack=true /p:PlatformTarget=AnyCPU /m 2>&1) -join "`r`n"
