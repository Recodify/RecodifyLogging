...
#>
	[cmdletbinding()]
		Param(
			...
			[Parameter(Mandatory = $False )]
				[string]
				$searchString = "Tests.Unit",		
			[Parameter(Mandatory = $False )]
				[string]
				$testResultsOutputDirectory = "",
			[Parameter(Mandatory = $False )]
				[string]
				$excludedCategories = ""		
			)
	...
	Process {
				...
				if ($testResultsOutputDirectory -eq "")
				{
					$testResultsOutputDirectory = $basePath
				}
				
				if (Test-Path "$testResultsOutputDirectory\TestResult_$($searchString)*.xml")
				{
					Write-Warning "Removing all previous test result file(s) matching: $testAssemblyPath\TestResult*.xml"
					Remove-Item "$testResultsOutputDirectory\TestResult_$($searchString)*.xml" -Force -ErrorAction SilentlyContinue
				}
				else
				{
					mkdir $testResultsOutputDirectory
				}
				...
				foreach($testAssemblyPath in $testAssemblyPaths)
				{
					...
					{	
							...
							if ($excludedCategories -ne "")
							{
							    & $nUnitPath $testAssemblyPath /process:multiple /exclude:$excludedCategories /result=$($testResultsOutputDirectory)\$($testResultFileName)
							}
							else
							{ 
							    & $nUnitPath $testAssemblyPath /process:multiple /result=$($testResultsOutputDirectory)\$($testResultFileName)
							}
							Write-Host "##teamcity[importData type='nunit' path='$($testAssemblyPath)\$($testResultFileName)']"
